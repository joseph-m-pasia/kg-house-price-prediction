...
logger.info(
    f"task_04_generate_final_results.py - Making predictions with the champion model: {champion_model_name} ..."
)
...
logger.info(
    "task_04_generate_final_results.py - Saving final predictions in CSV format in the output directory {}.".format(
        output_path_data
    )
)
results_df = pd.DataFrame({"Id": test_data["Id"], "SalePrice": y_pred_original_scale})
# check if folder exists, if not create it
if not (PROJECT_ROOT / output_path_data).exists():
    logger.info(
        f"task_04_generate_final_results.py - Output directory {output_path_data} does not exist. Creating it..."
    )
    (PROJECT_ROOT / output_path_data).mkdir(parents=True, exist_ok=True)
else:
    logger.info(
        f"task_04_generate_final_results.py - Output directory {output_path_data} already exists. Saving results..."
    )
