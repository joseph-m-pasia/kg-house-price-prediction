...
def optimize_nb_trees(X_train, y_train):
    ...
    for train_index, val_index in kf.split(X_train_processed):
        ...
        xgb_model = XGBRegressor(**params, n_estimators=5000)
...
def tune_complexity_parameters(X_train, y_train, n_estimators):
    ...
    model = Pipeline(
        steps=[
            ...
            (
                "regressor",
                XGBRegressor(objective="reg:squarederror", eval_metric="rmse", n_estimators=n_estimators, seed=42),
            ),
        ]
    )

    param_grid = {"regressor__max_depth": [3, 5, 7], "regressor__min_child_weight": [1, 3, 5]}
...
def tune_regalpha_reglambda(X_train, y_train, n_estimators, max_depth, min_child_weight, gamma):
    ...
    param_grid = {"regressor__reg_alpha": [0, 0.1, 0.5, 1], "regressor__reg_lambda": [0.5, 1, 1.5, 2]}
...
def tune_subsample_colsample(X_train, y_train, n_estimators, max_depth, min_child_weight, gamma, reg_alpha, reg_lambda):
    ...
    param_grid = {"regressor__subsample": [0.6, 0.8, 1.0], "regressor__colsample_bytree": [0.6, 0.8, 1.0]}

    grid_search = GridSearchCV(model, param_grid, cv=CONFIG["params"]["cv_size"], scoring="r2", n_jobs=-1)
    grid_search.fit(X_train, y_train)

    logger.info(
        f"tune_subsample_colsample() - Best subsample and colsample parameters found: {grid_search.best_params_}"
    )
...
def tune_learning_rate(
    ...
    logger.info(
        f"tune_learning_rate() - Best learning rate found: {grid_search.best_params_['regressor__learning_rate']}"
    )
...
if __name__ == "__main__":
    ...
    best_gamma = tune_gamma_parameter(
        X_train,
        y_train,
        avg_n_estimators,
        best_complexity_params["regressor__max_depth"],
        best_complexity_params["regressor__min_child_weight"],
    )

    best_reg_params = tune_regalpha_reglambda(
        X_train,
        y_train,
        avg_n_estimators,
        best_complexity_params["regressor__max_depth"],
    ...
    best_subsample_colsample = tune_subsample_colsample(
        X_train,
        y_train,
        avg_n_estimators,
        best_complexity_params["regressor__max_depth"],
        best_complexity_params["regressor__min_child_weight"],
        best_gamma,
        best_reg_params["regressor__reg_alpha"],
        best_reg_params["regressor__reg_lambda"],
    )

    best_learning_rate = tune_learning_rate(
        X_train,
        y_train,
        avg_n_estimators,
        best_complexity_params["regressor__max_depth"],
        ...
        best_reg_params["regressor__reg_alpha"],
        best_reg_params["regressor__reg_lambda"],
        best_subsample_colsample["regressor__subsample"],
        best_subsample_colsample["regressor__colsample_bytree"],
    )
